...
Temp_end    <- 37.0  # Temperature for last data point
...
DPGrbc_global <- 0.00350 # DPGrbc values for whole code
...
lower_loa <- round(mean_diff - 1.96 * sd(combined_data$Difference, na.rm = TRUE), 1)
...
combined_figure_3CD <- Figure_4_updated_final / Figure_3 +
  plot_layout(heights = c(17, 5.32)) #+ # Adjust relative heights accordingly
  #plot_annotation(tag_levels = list(c('C', 'D'))) # Custom labels for panels

# Save the combined figure
ggsave(
  filename = "Combined_Figure_3CD.tiff",
  plot = combined_figure_3CD,
...
# Display the combined figure
print(combined_figure_3CD)


# Merge Figure 4A with Figure 4B -----------------------------------------------
# Read the TIFF images
image1 <- image_read("Combined_Figure_3AB.tiff")
image2 <- image_read("Combined_Figure_3CD.tiff")

# Annotate with labels
image1_labeled <- image_annotate(
  image1,
  text = "A",                   # Label
  size = 150,                    # Font size (adjust as needed)
  gravity = "NorthWest",         # Position (top-left corner)
  location = "+50+50",           # Margin from top-left
  color = "black"                # Font color
)

# Then annotate C lower down
image1_labeled <- image_annotate(
  image1_labeled,
  text = "B",
  size = 150,
  gravity = "NorthWest",
  location = "+50+3700",   # adjust based on your image height
  color = "black"
)

image2_labeled <- image_annotate(
  image2,
  text = "C",
  size = 150,
  gravity = "NorthWest",
...
combined_image_3ABCD <- image_append(c(image1_labeled, image2_labeled), stack = FALSE)

# Save the combined image
image_write(combined_image_3ABCD, "Combined_Figure_3ABCD.tiff", format = "tiff")

# Optionally display the final image
print(combined_image_3ABCD)
